...
def fetch_and_clean_data(symbol: str, period: str = "5y") -> pd.DataFrame:
    ...
    try:
        ...
        # Ensure date is in proper YYYY-MM-DD format suitable for MySQL DATE type
...
def upsert_to_mysql(df: pd.DataFrame, db_config: dict):
    """
    Connects to MySQL, creates the table if it doesn't exist, and upserts data.
    """
    if df.empty:
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            logging.info("Upserting data into MySQL...")
...
if __name__ == "__main__":
    ...
    # Step 2: Upsert the data into the local MySQL database
    upsert_to_mysql(stock_df, DB_CONFIG)
